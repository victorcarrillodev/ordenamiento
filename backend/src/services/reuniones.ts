...
export async function listReuniones(): Promise<Reunion[]> {
  return sql<Reunion[]>`
    SELECT id, titulo, fecha::text AS fecha, hora_inicio, hora_fin
    FROM reuniones
...
export async function createReunion(input: {
  ...
  const rows = await sql<Reunion[]>`
    ...
    RETURNING id, titulo, fecha::text AS fecha, hora_inicio, hora_fin
  `
  return rows[0]
}

export async function deleteReunion(id: number): Promise<boolean> {
