...
function mockParticipacionesBackend(capture: {
  url: string | null
  response: { items: unknown[]; total: number; page: number; limit: number }
}) {
...
describe('Paginación participaciones · fuzz page/limit', () => {
  ...
  for (const c of fuzzCases) {
    it(`fuzz ${c.label} → page=${c.expectedPage} limit=${c.expectedLimit}`, async () => {
      const capture: {
        ...
        url: null,
        response: { items: [], total: 0, page: c.expectedPage, limit: c.expectedLimit },
      }
  ...
  it('page/limit con espacios y caracteres raros → fallback', async () => {
    ...
    const res = await router.fetch(
      new Request('http://localhost/ordena/admin/participaciones?page=%20%201%20&limit=%20'),
    )
    expect(res?.status).toBe(200)
...
describe('Paginación · renderizado condicional', () => {
  ...
  it('total=5 limit=10 → totalPages=1 → muestra meta pero NO nav', async () => {
    ...
    mockParticipacionesBackend(capture)
    const res = await router.fetch(
      new Request('http://localhost/ordena/admin/participaciones?page=1&limit=10'),
    )
    expect(res?.status).toBe(200)
    const html = await res?.text()
  ...
  it('total=25 limit=10 → totalPages=3 → muestra nav con Anterior/Siguiente', async () => {
    ...
    mockParticipacionesBackend(capture)
    const res = await router.fetch(
      new Request('http://localhost/ordena/admin/participaciones?page=2&limit=10'),
    )
    expect(res?.status).toBe(200)
    const html = await res?.text()
