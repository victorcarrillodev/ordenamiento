...
describe('Admin · preservación de valores tras error', () => {
  ...
  it('municipio del aporte vacío + datos del participante → fallback backend pero municipio_participante se repinta', async () => {
    ...
    expect((captured as unknown as FormData)?.get('municipio_participante')).toBe('Zapopan')
  })
  ...
  it('los 17 campos se repintan tras 502 (backend caído)', async () => {
    ...
    const html = await r?.text()
    for (const val of ['Nombre Largo', 'correo@ejemplo.com', 'Domicilio Part', 'Guadalajara', 'Calle Aporte', 'Colonia Aporte', 'Tonalá', '45400', '20.5', '-103.3', 'Inst X', 'Ingeniero', 'Observación detallada']) {
