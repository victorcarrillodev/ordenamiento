...
function mockAdminAuthAndBackend(opts: {
  ...
  globalThis.fetch = vi.fn().mockImplementation((url: string | URL | Request, init?: RequestInit) => {
    ...
    if (u.includes('/api/auth/me')) {
      return Promise.resolve(new Response(JSON.stringify({ user: { id: 1, name: 'Admin', role: 'admin', email: 'admin@tlaquepaque.gob.mx' } }), { status: 200, headers: { 'content-type': 'application/json' } }))
    }
    ...
    if (u.includes('/api/participations')) return Promise.resolve(new Response(JSON.stringify({ items: [] }), { status: 200 }))
...
describe('Personalización · testMail seguridad', () => {
  ...
  it('rechaza para vacío sin llamar a backend', async () => {
    ...
    const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm('') }))
  ...
  it('rechaza para sin @ sin llamar a backend', async () => {
    const capture = { para: null as string | null, called: false }
    mockAdminAuthAndBackend({ captureMailTest: capture })
    const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm('sin-arroba') }))
    expect(res?.status).toBe(302)
    expect(res?.headers.get('location')).toContain('err=')
    expect(capture.called).toBe(false)
  })
  ...
  it('rechaza CRLF injection en para (\r\n) sin llegar a backend', async () => {
    const capture = { para: null as string | null, called: false }
    mockAdminAuthAndBackend({ captureMailTest: capture })
    const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm('a@b.com\r\nX-Injected: 1') }))
  ...
  it('rechaza XSS/script en para', async () => {
    ...
    const payloads = [
      ...
      'a@b.com<script>',
      'a@b.com%0D%0A<script>',
    ]
    for (const p of payloads) {
      capture.called = false
      const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm(p) }))
      expect(res?.status).toBe(302)
  ...
  it('acepta email válido y llama al backend', async () => {
    const capture = { para: null as string | null, called: false }
    mockAdminAuthAndBackend({ captureMailTest: capture, mailTestResponse: { status: 200, body: { ok: true } } })
    const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm('admin@tlaquepaque.gob.mx') }))
    expect(res?.status).toBe(302)
  ...
  it('backend 503 (SMTP no configurado) → redirect err con mensaje', async () => {
    const capture = { para: null as string | null, called: false }
    mockAdminAuthAndBackend({ captureMailTest: capture, mailTestResponse: { status: 503, body: { error: 'Correo no configurado: define SMTP_HOST, SMTP_USER y SMTP_PASS' } } })
    const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm('test@example.com') }))
    expect(res?.status).toBe(302)
  ...
  it('backend 502 (fallo envío) → redirect err', async () => {
    mockAdminAuthAndBackend({ mailTestResponse: { status: 502, body: { error: 'No se pudo enviar prueba: ECONNREFUSED' } } })
    const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm('test@example.com') }))
    expect(res?.status).toBe(302)
    expect(res?.headers.get('location')).toContain('err=')
  })
  ...
  it('no autenticado → redirect a login y no llama a backend mail', async () => {
    globalThis.fetch = vi.fn().mockImplementation((url: string | URL | Request) => {
      const u = typeof url === 'string' ? url : url instanceof Request ? url.url : url.toString()
      if (u.includes('/api/auth/me')) return Promise.resolve(new Response(JSON.stringify({ user: null }), { status: 401 }))
      return Promise.resolve(new Response(JSON.stringify({ ok: true }), { status: 200 }))
    }) as unknown as typeof fetch
    const res = await router.fetch(new Request('http://localhost/ordena/admin/personalizacion', { method: 'POST', body: buildTestMailForm('a@b.com') }))
    expect(res?.status).toBe(302)
