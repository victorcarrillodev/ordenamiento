...
export function ActividadesPage(handle: Handle<ActividadesPageProps>) {
  return () => {
    const { theme, actividades, estado } = handle.props
    const isProximas = estado !== 'realizadas'
    return (
      <Document title="Actividades – POETDUM">
        <NavBar theme={theme} />
        <main mix={pageWrap}>
          <h1
            mix={css({
          ...
          <nav aria-label="Filtrar actividades" mix={tabBar}>
            <a
              href={`${routes.poetdum.actividades.show.href()}?estado=proximas`}
            ...
            <a
              href={`${routes.poetdum.actividades.show.href()}?estado=realizadas`}
              mix={!isProximas ? tabActive : tabInactive}
              aria-current={!isProximas ? 'page' : undefined}
            >
                  ...
                  {a.documentos && a.documentos.length > 0 ? (
                    <div mix={css({ display: 'flex', flexWrap: 'wrap', gap: '8px' })}>
                      {a.documentos.map((d) => (
                        <a
                          key={d.id}
                          href={routes.poetdum.documentos.show.href()}
                          mix={css({
                            display: 'inline-flex',
                            alignItems: 'center',
                            gap: '6px',
